...
// ============================================================
// Official breed combo definitions
// ============================================================

export const BREED_COMBOS: ProductCombo[] = [
  // ── 犬組合 ──
  {
    id: 'fluffy_styling',
    name: '蓬鬆造型組合',
  ...
  },
  {
    id: 'cat_fluffy',
    name: '蓬鬆回彈組合',
    description: '長毛蓬鬆立體，短毛彈性回彈',
    target_breeds: ['ragdoll', 'maine_coon', 'british_sh'],
    target_needs: '追求蓬鬆回彈效果的貓咪',
    steps: [
      { step: 1, role: '清潔', product_key: 'heavy_duty_clean', note: '強效洗劑或泡沫柑橘' },
      { step: 2, role: '功能', product_key: 'natural_elastic', note: '長毛用洋李洗劑，短毛用櫻桃漿果洗劑' },
      { step: 3, role: '護毛素', product_key: 'spa_deep_clean', note: '有掉毛問題用防脫毛乳液，追求蓬鬆用純粹護毛素' },
    ],
    tips: '短毛貓建議搭配櫻桃漿果洗劑',
  },
  {
    id: 'cat_smooth',
    name: '柔順服帖組合',
    description: '長毛柔順服帖，減少毛髮糾結',
    target_breeds: ['maine_coon', 'persian', 'chinchilla'],
    target_needs: '長毛貓追求柔順服帖',
    steps: [
      { step: 1, role: '清潔', product_key: 'heavy_duty_clean', note: '強效洗劑或泡沫柑橘' },
      { step: 2, role: '功能', product_key: 'detangling', note: '柔順洗劑' },
...
export function getBreedCombo(breed: string, color?: string | null): ProductCombo {
  ...
  if (c.includes('黑') || c.includes('black')) {
    return BREED_COMBOS.find((cb) => cb.id === 'black_coat')!;
  }
  if (c.includes('白') || c.includes('white') || c.includes('奶油') || c.includes('cream')) {
    return BREED_COMBOS.find((cb) => cb.id === 'white_coat')!;
  }
  if (c.includes('紅') || c.includes('red') || c.includes('棕') || c.includes('杏') || c.includes('apricot') || c.includes('橘') || c.includes('orange')) {
    return BREED_COMBOS.find((cb) => cb.id === 'red_brown_coat')!;
  }

  // Breed-based matching — find combo with this breed in target_breeds
  const breedId = resolveBreedId(b);
  if (breedId) {
    const match = BREED_COMBOS.find((cb) => cb.target_breeds.includes(breedId));
    if (match) return match;
  }

  // Fallback: detect cat vs dog for premium default
  const isCat = b.includes('貓') || b.includes('cat') || ['british_sh', 'american_sh', 'russian_blue', 'scottish_fold', 'ragdoll', 'maine_coon', 'persian', 'chinchilla', 'exotic_sh', 'kitten'].includes(breedId ?? '');
  return BREED_COMBOS.find((cb) => cb.id === (isCat ? 'cat_premium' : 'dog_premium'))!;
}

/** Get all combos for a specific breed (may match multiple) */
export function getCombosForBreed(breedId: string): ProductCombo[] {
  return BREED_COMBOS.filter((cb) => cb.target_breeds.includes(breedId));
}
...
export function getPairedProducts(productId: string): DavisProduct[] {
  ...
  for (const combo of BREED_COMBOS) {
