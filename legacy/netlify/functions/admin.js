...
function hashPw(pw) { return crypto.createHash("sha256").update((process.env.PASSWORD_SALT || "davis_salt_") + pw).digest("hex"); }
...
exports.handler = async (event) => {
  ...
  function makeToken(username, role) {
    ...
    const sig = crypto.createHash("sha256").update(username + ":" + role + ":" + exp + ":" + (process.env.DAVIS_TOKEN_SECRET || (process.env.TOKEN_SECRET || "davis_token_secret_change_me"))).digest("hex").slice(0, 16);
  ...
  if (rawToken) {
    try {
      ...
      if (decoded.exp && decoded.exp < now) { /* expired */ }
        ...
        const expectedSig = crypto.createHash("sha256").update(decoded.username + ":" + decoded.role + ":" + decoded.exp + ":" + (process.env.DAVIS_TOKEN_SECRET || (process.env.TOKEN_SECRET || "davis_token_secret_change_me"))).digest("hex").slice(0, 16);
  ...
  if (path === "/change-password" && method === "POST") {
    ...
    if (!new_password || new_password.length < 8) return json(400, { error: "新密碼至少 8 字元" });
  ...
  if (path === "/settings" && method === "GET") {
    ...
    const settings = {}; (r.data || []).forEach((s) => {
  ...
  if (path === "/settings" && method === "PUT") {
    const ALLOWED_SETTING_KEYS = [
      ...
      "ai_pricing", "anthropic_api_key", "site_title", "site_description",
      "contact_line", "contact_email", "certify_enabled", "share_enabled",
    ];
    for (const [key, value] of Object.entries(body)) {
      if (!ALLOWED_SETTING_KEYS.includes(key)) continue; // silently ignore unknown keys
  ...
  if (path === "/users" && method === "POST") {
    ...
    const r = await supaReq("POST", "/davis_users", { username, password_hash: hashPw(password), role: safeRole, display_name: display_name || username, updated_at: new Date().toISOString() }, SUPA_KEY, SUPA_URL);
